...
def third_central_moment(X: torch.Tensor) -> torch.Tensor:
    ...
    CENTERING CONVENTION (pinned): each sample is centered about ITS OWN mean.
    Central moments are location-invariant, so a pure mean shift between two
    populations contributes NOTHING to their cumulant difference at any order
    -- mean-shifted Gaussians are indistinguishable to every kernel in this
    module by construction (verified numerically). Location effects are the
    aggregation-contrast instrument's job (lmoment.aggregation_contrast_test),
    not this module's."""
...
def fourth_central_moment(X: torch.Tensor) -> torch.Tensor:
    ...
    Cumulant additivity (Zwiernik arXiv:1011.1722) holds exactly:
    independent X,Y ⇒ cross-block κ_4 = 0. X:[n,d] → [d,d,d,d].
    Conformance: Zwiernik arXiv:1011.1722; standard cumulant definition."""
    n = X.shape[0]
    Xc = X - X.mean(dim=0, keepdim=True)
...
def cross_cumulant_residual_perK(X: torch.Tensor, Y: torch.Tensor,
    ...
    Conformance: axioms §0.5; Zwiernik arXiv:1011.1722 (cumulants of
...
def admit_per_component(X: torch.Tensor, Y: torch.Tensor,
    ...
    SCOPE (pinned): this gate tests CROSS-BLOCK DEPENDENCE between co-observed
    feature blocks X and Y (cumulants of independent variables are additive,
    so the cross-block residual is the dependence signature). It is NOT a
    between-population divergence test: for two INDEPENDENT samples the
    cross-cumulants vanish regardless of how different the two shapes are
    (verified numerically). For population shape divergence use
    cumulant_difference.

    DELIBERATE INSENSITIVITY (documented trade): the strict AND means
    dependence expressed at ONE order only is never admitted -- the
    anti-masking design (a large order-3 signal cannot carry a null order-4
    signal through) symmetrically rejects a real order-3-only or
    order-4-only dependence. blocking_component names the order that
    blocked, so single-order signals are reported even when not admitted."""
...
def cumulant_difference(X: torch.Tensor, Y: torch.Tensor,
    ...
    NO BUILT-IN NULL (pinned): this returns a raw statistic with no admission
    gate or calibration. Its magnitude is scale- and n-dependent; interpret it
    only against a permutation/resampling null computed by the caller (the
    executed notebooks report it as a real-to-null ratio for exactly this
    reason). Do not threshold the raw value.
